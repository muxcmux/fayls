        ...
        html {
            ...
            body {
                main x-data="{ search_q: new URLSearchParams(location.search).get('q') }" {
                    form hx-get="/search" hx-push-url="true" hx-target="#file_list" hx-swap="outerHTML" {
...
fn file_list_header(
    ...
    html! {
        th.(col.as_str()).asc[asc].desc[desc] hx-push-url="true" hx-target="#file_list" hx-swap="outerHTML" hx-get=(queries_to_string(&queries)) {
        ...
        section #file_list {
            @if !crumbs.is_empty() {
                nav {
                    ul {
                        li {
                            a href={ "/" (&query_string) } x-on:click="search_q = ''" hx-get={ "/" (&query_string) } hx-target="#file_list" hx-swap="outerHTML" hx-push-url="true" {
                ...
                tbody {
                    @if files.is_empty() {
                        ...
                        @for file in files {
                            ...
                            tr x-on:click="search_q = ''" hx-get=(link) hx-target="#file_list" hx-swap="outerHTML" hx-push-url="true" {
